...
class TwitterPlugin:
    def __init__(self, options: Dict[str, Any]) -> None:
        ...
        self._functions: Dict[str, Callable[..., Any]] = {
            ...
            "reply_tweet_long": self._reply_tweet_long,
    ...
    def _reply_tweet_long(self, tweet_id: int, reply_list: List[str]) -> None:
        """
        Reply to a tweet with multiple messages, each message being a part of a longer reply.

        Args:
            tweet_id: The ID of the tweet to reply to.
            reply_list: A list of strings, where each string is a part of the reply to be tweeted
                        in sequence.

        Returns:
            None

        Logs an error if replying to any tweet fails.
        """
        try:
            previous_tweet_id = tweet_id
            for reply in reply_list:
                response = self.twitter_client.create_tweet(
                    in_reply_to_tweet_id=previous_tweet_id, text=reply
                )
